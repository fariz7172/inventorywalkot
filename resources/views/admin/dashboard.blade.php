...
<div class="mb-6">
    ...
    <p class="text-sm text-gray-500 mt-0.5">{{ now()->translatedFormat('l, d F Y') }} — Ringkasan operasional gudang Anda.</p>
</div>

{{-- ===== STAT CARDS ===== --}}
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    @foreach($stats as $s)
    <div class="bg-white rounded-2xl p-4 lg:p-5 shadow-card ring-1 {{ $s['ring'] }}">
        <div class="flex items-start justify-between mb-4">
            <div class="w-11 h-11 rounded-xl {{ $s['color'] }} flex items-center justify-center">
        ...
        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">{{ $s['label'] }}</p>
    </div>
    @endforeach
</div>
        ...
        <div class="space-y-6">
            ...
            <div class="flex gap-4 relative group">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-bold text-gray-800 truncate">{{ $act->material->name }}</p>
                    <p class="text-[11px] text-gray-500 font-medium">
                        {{ $act->type === 'in' ? 'Masuk' : 'Keluar' }}: 
                        <span class="font-black {{ $act->type === 'in' ? 'text-emerald-600' : 'text-red-500' }}">
                            {{ (float)($act->type === 'in' ? $act->volume_masuk : $act->volume_keluar) }} {{ $act->material->unit }}
                        </span>
                    </p>
                    <p class="text-[9px] text-gray-400 mt-0.5 uppercase font-bold tracking-tighter">{{ $act->created_at->diffForHumans() }}</p>
                </div>
            </div>
            @empty
