            ...
            <form wire:submit="processIncoming" class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" wire:model="reference_number" placeholder="SM-001" class="w-full bg-base rounded-2xl px-4 py-3 text-sm border-none focus:ring-2 focus:ring-accent/20 outline-none @error('reference_number') ring-2 ring-red-500/50 @enderror">
                        @error('reference_number') <span class="text-[10px] text-red-500 font-bold mt-1 ml-1">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-1.5 ml-1">Nama Supplier</label>
                        <input type="text" wire:model="supplier" placeholder="PT. Maju Jaya" class="w-full bg-base rounded-2xl px-4 py-3 text-sm border-none focus:ring-2 focus:ring-accent/20 outline-none @error('supplier') ring-2 ring-red-500/50 @enderror">
                        @error('supplier') <span class="text-[10px] text-red-500 font-bold mt-1 ml-1">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-base/40 rounded-3xl p-4 ring-1 ring-warm/30 relative group">
                        @if(count($items) > 1)
                        <button type="button" wire:click="removeItem({{ $index }})" class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center shadow-lg shadow-red-500/20 hover:bg-red-600 transition-all opacity-0 group-hover:opacity-100 z-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            <div class="md:col-span-6">
                                ...
                                <select wire:model="items.{{ $index }}.material_id" class="w-full bg-white rounded-xl px-4 py-2.5 text-sm border-none focus:ring-2 focus:ring-accent/20 outline-none shadow-sm @error('items.'.$index.'.material_id') ring-2 ring-red-500/50 @enderror">
                                    <option value="">-- Pilih Barang --</option>
                                    @foreach($allMaterials as $mat)
                                        <option value="{{ $mat->id }}">{{ $mat->name }} ({{ $mat->unit }})</option>
                                    @endforeach
                                </select>
                                @error('items.'.$index.'.material_id') <span class="text-[9px] text-red-500 font-bold mt-1 ml-1">Harus dipilih</span> @enderror
                            </div>
                            <div class="md:col-span-3">
                                <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1">Jumlah</label>
                                <input type="number" step="any" wire:model="items.{{ $index }}.volume" placeholder="0" class="w-full bg-white rounded-xl px-4 py-2.5 text-sm border-none focus:ring-2 focus:ring-accent/20 outline-none shadow-sm @error('items.'.$index.'.volume') ring-2 ring-red-500/50 @enderror">
                                @error('items.'.$index.'.volume') <span class="text-[9px] text-red-500 font-bold mt-1 ml-1">Minimal 0.01</span> @enderror
                            </div>
                            <div class="md:col-span-3">
                                <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1">Keterangan</label>
                                <input type="text" wire:model="items.{{ $index }}.note" placeholder="..." class="w-full bg-white rounded-xl px-4 py-2.5 text-sm border-none focus:ring-2 focus:ring-accent/20 outline-none shadow-sm">
                            </div>
                        </div>
                    </div>
                    @endforeach
                ...
                <div>
                    ...
                    @error('photo') <span class="text-[10px] text-red-500 font-bold mt-1 ml-1">{{ $message }}</span> @enderror
