...
<head>
    ...
    <title>@yield('title', 'Dashboard') — {{ \App\Models\Setting::get('app_name', 'AdminPro') }}</title>
            ...
            <div class="flex items-center gap-3 px-5 py-5 border-b border-warm/60">
                ...
                @if($appLogo)
                    <div class="w-9 h-9 rounded-xl bg-white flex items-center justify-center flex-shrink-0 shadow-sm overflow-hidden p-0.5 border border-gray-100">
                        <img src="{{ asset($appLogo) }}" alt="Logo" class="w-full h-full object-cover rounded-lg">
                    </div>
                @else
                        ...
                        <div class="relative" x-data="{ openNotif: false }">
                            ...
                            <div x-show="openNotif" style="display: none;"
                                ...
                                <div class="px-4 py-3 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                                    ...
                                    @if($notifCount > 0)
                                    <span class="bg-accent/10 text-accent text-[10px] font-bold px-2 py-0.5 rounded-md">{{ $notifCount }} Baru</span>
                                    @endif
                                </div>
                                ...
                                <div class="max-h-80 overflow-y-auto">
                                    @if($notifItems->count() > 0)
                                        @foreach($notifItems as $item)
                                            <a href="/dashboard/surat-jalan/{{ $item->id }}" class="block px-4 py-3 hover:bg-gray-50 border-b border-gray-50 transition-colors">
                                                <div class="flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-[10px] text-gray-400 mt-1.5 font-medium">{{ $item->updated_at->diffForHumans() }}</p>
                                                    </div>
                                                </div>
                                            </a>
                                        @endforeach
